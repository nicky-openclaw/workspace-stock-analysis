...
STEPS = [
    {
        "name": "Step 1: 图片识别",
        "script": "ztx_step2_fetch.py",
        "output": "ztx_stocks.json",
        "desc": "读取股票代码清单（需手动提供或由图片识别生成）"
    },
    {
        "name": "Step 2: K线数据拉取",
        "script": "ztx_step2_fetch.py",
        "output": "ztx_kline.json",
        "desc": "拉取120日K线，沪市用sh前缀，深市用sz前缀"
    },
    {
        "name": "Step 3: 指标计算",
        "script": "ztx_step3_calc.py",
        "output": "ztx_indicators.json",
        "desc": "计算MA5/MA10/MA20/J值/RSI/量比等指标"
    },
    {
        "name": "Step 4: 综合评分",
        "script": "ztx_step4_score.py",
        "output": "ztx_scores.json",
...
def main():
    print("🏗️  砖型图选股主控脚本 v2.0")
    print(f"输出目录: {OUTPUT_DIR}")
    OUTPUT_DIR.mkdir(exist_ok=True)

    # 检查 ztx_stocks.json 是否已存在（Step 1 由Agent手动提供）
    stocks_file = OUTPUT_DIR / "ztx_stocks.json"
    if not stocks_file.exists():
        print("\n⚠️  ztx_stocks.json 不存在！")
        print("请先运行图片识别，生成股票代码清单。")
        print("格式参考: scripts/ztx_stocks_template.json")
        sys.exit(1)

    ok, msg = check_output("ztx_stocks.json")
    print(f"\n股票清单: {msg}")
    ...
    print(f"\n{'='*60}")
    print("✅ 所有步骤完成！")
    scores_path = OUTPUT_DIR / "ztx_scores.json"
